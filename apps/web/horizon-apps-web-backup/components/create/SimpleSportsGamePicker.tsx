...
interface SimpleSportsGamePickerProps {
  league: League;
  selectedGame?: Game;
...
export function SimpleSportsGamePicker({ 
  league, 
  selectedGame, 
  onGameSelect, 
  onLeagueChange 
}: SimpleSportsGamePickerProps) {
  
  console.log('SimpleSportsGamePicker loaded with latest changes');
  ...
  const [activeFilter, setActiveFilter] = useState('earliest');
  ...
  const filterChips: FilterChip[] = useMemo(() => [
    { id: 'earliest', label: 'Upcoming' }, // Special filter for earliest available games
    ...
    { id: 'next30d', label: 'Next 30d', dateRange: { startUTC: new Date(), endUTC: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) } },
  ...
  }, [league, activeDateRange.startUTC.getTime(), activeDateRange.endUTC.getTime(), includeLive, includePostponed]);
  ...
  const { displayedGames, totalCount, hasMore } = useMemo(() => {
    ...
    // For "Upcoming" filter, show all games sorted by date (no limit)
    if (activeFilter === 'earliest' && games.length > 0) {
      // Sort all games by start time to show them in chronological order
  ...
  }, [games, searchTerm, currentPage, activeFilter]);
